...
export function SmsPage() {
  ...
  async function saveDesignation(event: FormEvent) {
    ...
    setMessage('Safety Manager / Safety Representative saved.');
  }
  ...
  return (
    <section className="mx-auto flex w-full max-w-4xl flex-col gap-4">
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <h2 className="text-lg font-semibold text-brand-900">Safety Responsibility</h2><p className="mt-1 text-sm text-slate-600">Designate an active organization member as the Safety Manager / Safety Representative. A person may also serve as RPIC.</p>
        {loading ? <p className="mt-4 text-sm text-slate-500">Loading safety responsibility...</p> : <form className="mt-4 space-y-3" onSubmit={saveDesignation}><label className="block text-sm font-medium text-slate-700">Safety Manager / Safety Representative<select className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-3 text-base sm:py-2 sm:text-sm" value={selectedId} onChange={(event) => setSelectedId(event.target.value)} disabled={!canManage || saving}><option value="">Select an active member</option>{members.map((member) => <option key={member.id} value={member.id}>{member.full_name} — Safety Manager / Safety Representative{isRpic(member) ? ' + RPIC' : ` (${member.role})`}</option>)}</select></label>{selected ? <p className="rounded-lg bg-brand-50 px-3 py-2 text-sm font-medium text-brand-900">{selected.full_name} — Safety Manager / Safety Representative{isRpic(selected) ? ' + RPIC' : ''}</p> : null}{canManage ? <button className="min-h-11 rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400" disabled={saving || !selectedId}>{saving ? 'Saving...' : 'Save Designation'}</button> : <p className="text-sm text-slate-500">Only the organization owner can change this designation.</p>}</form>}
      </article>
      <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
