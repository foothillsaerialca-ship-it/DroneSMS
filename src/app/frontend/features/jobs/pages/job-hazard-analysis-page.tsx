  ...
  useEffect(() => {
    ...
    async function loadJha() {
      ...
      try {
        ...
        setAttestations({ safetyManagerReviewedAt: assessment?.safety_manager_reviewed_at ?? null, safetyManagerName: assessment?.safety_manager_name ?? null, rpicAcceptedAt: assessment?.rpic_accepted_at ?? null, rpicName: assessment?.rpic_name ?? null });
...
function CertificationStep({ formData, updateField, togglePpe, toggleFormArrayField, isSaving, jobId, safetyDesignation, rpic, currentUserId, attestations, onAttest }: StepProps & { togglePpe: (option: string) => void; toggleFormArrayField: (field: 'runoffContaminants' | 'communicationMethods', value: string) => void; jobId: string; safetyDesignation: SafetyDesignation; rpic: RolePerson | null; currentUserId: string; attestations: JhaAttestations; onAttest: (kind: 'safety-manager' | 'rpic') => Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-3"><p className="text-sm text-slate-600">Record what the crew and RPIC confirmed before operations begin.</p><div className="mt-4 space-y-3"><Checkbox label="Crew briefed on hazards, controls, PPE, emergency procedures, communications, and stop-work authority" checked={formData.crewBriefed} onChange={(checked) => updateField('crewBriefed', checked)} disabled={isSaving} /><Checkbox label="Controls are in place before operations begin" checked={formData.controlsInPlace} onChange={(checked) => updateField('controlsInPlace', checked)} disabled={isSaving} /><Checkbox label="RPIC stop-work authority acknowledged" checked={formData.stopWorkAuthorityAcknowledged} onChange={(checked) => updateField('stopWorkAuthorityAcknowledged', checked)} disabled={isSaving} /></div></div>
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="rounded-lg border border-slate-200 bg-white p-4"><h3 className="text-sm font-semibold uppercase tracking-wide text-brand-900">Safety Manager Review</h3>{safetyManager ? <><p className="mt-2 font-semibold text-slate-900">{safetyManager.full_name} — {operationalRoleLabel(true, dualRole)}</p><p className="mt-3 text-sm text-slate-600">I have reviewed this Operational JHA, including the identified hazards, controls, crew briefing information, and applicable operational safety considerations.</p><p className="mt-3 text-xs font-medium text-slate-500">Status: {attestations.safetyManagerReviewedAt ? `Reviewed by ${attestations.safetyManagerName ?? safetyManager.full_name} on ${formatAttestationTime(attestations.safetyManagerReviewedAt)}` : 'Pending review'}</p><button type="button" className="mt-4 min-h-11 rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400" onClick={() => void onAttest('safety-manager')} disabled={isSaving || currentUserId !== safetyManager.user_id}>{attestations.safetyManagerReviewedAt ? 'Review Again' : 'Complete Safety Manager Review'}</button>{currentUserId !== safetyManager.user_id ? <p className="mt-2 text-xs text-slate-500">Only the designated Safety Manager can complete this review while signed in.</p> : null}</> : <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p>Safety Manager not designated. Configure a Safety Manager in SMS.</p><Link className="mt-2 inline-block font-semibold text-brand-700" to="/sms">Configure Safety Manager</Link></div>}</section>
      <section className="rounded-lg border border-slate-200 bg-white p-4"><h3 className="text-sm font-semibold uppercase tracking-wide text-brand-900">RPIC Acceptance</h3>{rpic ? <><p className="mt-2 font-semibold text-slate-900">{rpic.full_name} — {operationalRoleLabel(dualRole, true)}</p><p className="mt-3 text-sm text-slate-600">I have reviewed the Operational JHA, hazards, controls, crew briefing, emergency and communication information, and accept responsibility for conducting the operation under the established controls.</p><p className="mt-3 text-xs font-medium text-slate-500">Status: {attestations.rpicAcceptedAt ? `Accepted by ${attestations.rpicName ?? rpic.full_name} on ${formatAttestationTime(attestations.rpicAcceptedAt)}` : 'Pending acceptance'}</p><button type="button" className="mt-4 min-h-11 rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white disabled:bg-slate-400" onClick={() => void onAttest('rpic')} disabled={isSaving || currentUserId !== rpic.user_id}>{attestations.rpicAcceptedAt ? 'Accept Again' : 'Accept as RPIC'}</button>{currentUserId !== rpic.user_id ? <p className="mt-2 text-xs text-slate-500">Only the assigned RPIC can complete this acceptance while signed in.</p> : null}</> : <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p>RPIC not assigned to this job.</p><Link className="mt-2 inline-block font-semibold text-brand-700" to={`/jobs/${jobId}/hub#crew-assignment`}>Assign RPIC</Link></div>}</section>
    </div>
