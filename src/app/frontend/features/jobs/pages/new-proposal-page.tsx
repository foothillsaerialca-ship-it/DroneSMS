    ...
    <section className="space-y-4">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <FormSection title="Airspace Review">
          <SelectField label="Airspace Classification" value={formData.airspaceClass} options={airspaceClasses} onChange={(value) => updateField('airspaceClass', value)} disabled={isFormDisabled} />
          <TextField label="Relevant Airport / Heliport" value={formData.relevantAirportHeliport} onChange={(value) => updateField('relevantAirportHeliport', value)} disabled={isFormDisabled} placeholder="Airport or heliport name, identifier, or None identified" />
          ...
          <TextField label="Known Airspace Restrictions / TFR Considerations" value={formData.knownAirspaceRestrictions} onChange={(value) => updateField('knownAirspaceRestrictions', value)} disabled={isFormDisabled} placeholder="Preliminary restrictions, TFR considerations, or None identified" />
        </FormSection>
...
function HazardSelection({
  ...
  const visibleHazards = useMemo(() => getVisibleHazards(hazardLibrary, serviceType, libraryView), [hazardLibrary, serviceType, libraryView]);
  const searchResults = useMemo(() => searchHazards(visibleHazards, searchQuery), [visibleHazards, searchQuery]);
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <h3 className="text-sm font-semibold text-brand-900">Hazard Library</h3>
              <span className="text-xs font-medium text-slate-500">{serviceType}</span>
            </div>
            <div className="mt-3 inline-flex rounded-lg border border-slate-300 bg-white p-1" aria-label="Hazard library view">
              {(['relevant', 'all'] as const).map((view) => (
                <button key={view} type="button" className={`min-h-10 rounded-md px-3 py-2 text-sm font-medium ${libraryView === view ? 'bg-brand-700 text-white' : 'text-slate-700 hover:bg-slate-50'}`} onClick={() => setLibraryView(view)} disabled={disabled} aria-pressed={libraryView === view}>
                  {view === 'relevant' ? 'Relevant Hazards' : 'All Hazards'}
                </button>
              ))}
            </div>
            <p className="mt-1 text-xs text-slate-600">
              {libraryView === 'all'
                ? 'The complete available library is shown. Viewing it does not select or change any hazards.'
            ...
            {visibleHazards.length ? (
              ...
              <div className="mt-3 rounded-lg border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-600">
                No hazards are available in this view.
              </div>
            )}
          </div>

          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <h3 className="text-sm font-semibold text-brand-900">Search {libraryView === 'relevant' ? 'Relevant Hazards' : 'All Hazards'}</h3>
            <input
