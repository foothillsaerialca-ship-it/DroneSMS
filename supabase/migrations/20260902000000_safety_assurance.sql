...
create function public.save_operation_closeout_with_assurance(
...
begin
  ...
  foreach related_id in array coalesce(related_jha_hazard_ids_value,'{}') loop
  ...
  foreach related_id in array coalesce(related_control_ids_value,'{}') loop
  ...
  if exists (select 1 from unnest(coalesce(related_safety_event_ids_value,'{}')) as related(event_id) left join public.job_safety_events event on event.id=related.event_id where event.id is null or event.job_id<>target_job.id or event.organization_id<>target_job.organization_id) then raise exception 'Related safety event is not part of this job'; end if;
  ...
  values(target_job.organization_id,target_job.id,saved_closeout.id,control_effectiveness_value,nullif(btrim(effectiveness_narrative_value),''),nullif(btrim(operational_action_value),''),unexpected_issue_value,nullif(btrim(unexpected_issue_narrative_value),''),follow_up_required_value,coalesce(follow_up_areas_value,'{}'),coalesce(related_jha_hazard_ids_value,'{}'),coalesce(related_control_ids_value,'{}'),coalesce(related_safety_event_ids_value,'{}'),case when should_open then 'Open' else 'Not Required' end,auth.uid(),previous_review_id,target_job.service_type,target_job.name);
